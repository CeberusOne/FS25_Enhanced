-- One selector for effort, not look. Colour, FOV, tone curve, moods, paint
-- and wetness stay on their own sliders.
...
function M.set(index, automatic)
    ...
    if index == 0 then
        ...
        if FS25E_ModSettings then FS25E_ModSettings.set('qualityLevel', M.index) end
        return ok
    end
    ...
    if FS25E_ModSettings then FS25E_ModSettings.set('qualityLevel', index) end
    if FS25E_Debug then
        FS25E_Debug.info('QualityLevels', string.format('level %d applied=%d skipped=%d failed=%d', index, applied, skipped, failed))
    end
...
function M.getControls()
    return { {
        id = 'enhancedLevel', category = 'presets', min = 0, max = 5, step = 1, kind = 'enum', cost = 'high',
