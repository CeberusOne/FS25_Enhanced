...
function FS25E_TelemetryReader.init()
    ...
    lastError = nil
    wallMs = 0
    -- FS25 permits io.open only for writing; do not probe a forbidden read.
    readDisabled = type(loadXMLFile)=="function" or g_currentMission~=nil
    disableLogged = false
    if readDisabled then lastError="External JSON telemetry is unavailable in the FS25 Lua sandbox" end
    FS25E_Debug.info("TelemetryReader", "init readDisabled=" .. tostring(readDisabled) .. " (mission frame statistics remain available)")
end
